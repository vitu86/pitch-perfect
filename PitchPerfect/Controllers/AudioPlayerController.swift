...
class AudioPlayerController: NSObject, AVAudioPlayerDelegate {
    ...
    private struct ErrorHelper {
        static let AudioFileError = NSLocalizedString(
            "AudioFileError", comment: "Message when audio file creation goes wrong")
        static let AudioEngineError = NSLocalizedString(
            "AudioEngineError", comment: "Message when audio engine start goes wrong")
    }
    ...
    init(
        fileURL: URL, functionToHandleErrors: @escaping ((String) -> Void),
        functionToHandleStates: @escaping ((StateHelper) -> Void)
    ) {
        super.init()
    ...
    func playSound(
        ...
        if echo == true && reverb == true {
            connectAudioNodes(
                audioPlayerNode, changeRatePitchNode, echoNode, reverbNode, audioEngine.outputNode)
        } else if echo == true {
            connectAudioNodes(
                audioPlayerNode, changeRatePitchNode, echoNode, audioEngine.outputNode)
        } else if reverb == true {
            connectAudioNodes(
                audioPlayerNode, changeRatePitchNode, reverbNode, audioEngine.outputNode)
        } else {
        ...
        audioPlayerNode.scheduleFile(audioFile, at: nil) {
            ...
            self.stopTimer = Timer(
                timeInterval: delayInSeconds, target: self, selector: #selector(self.stopAudio),
